...
def get_tag_name_by_id(tag_id):
    return model.Tag.query.filter(model.Tag.id == tag_id).one().name


def get_tag_id_by_name(tag_name):
    tag = model.Tag.query.filter(model.Tag.name == tag_name).first()
    if not tag:
        return False
    return tag.id
...
def get_tags_by_deckmark_id(deckmark_id):
    """Returns list of Tag database objects"""
    return model.Deckmark.query.filter(model.Deckmark.id == deckmark_id).one().tags
